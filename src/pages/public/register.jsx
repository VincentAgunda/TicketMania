...
const Register = () => {
  const { user, isAdmin, signUp, loading: authLoading } = useAuth()
  ...
  // Redirect if user is already logged in (same logic as Login component)
  useEffect(() => {
    if (user) {
        ...
        >
          ...
          <div>
            ...
            <div className="relative">
              ...
              >
                {showPassword ? ( <VisibilityOff className="h-5 w-5 text-gray-500"/> ) : ( <Visibility className="h-5 w-5 text-gray-500" /> )}
              </button>
            </div>
          </div>
          ...
          >
            {isRegistering ? <ButtonLoader text="Creating account..." /> : 'Create Account'}
          </button>
